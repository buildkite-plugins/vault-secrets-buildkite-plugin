...
[ $? -eq 0 ] && vault mount -path kv/buildkite kv

TESTDATA_1=`echo TESTDATA_1=\"foo bar 1\" | base64`
TESTDATA_2=`echo TESTDATA_2=\"foo bar 2\" | base64`

[ $? -ne 1 ] && {
   echo "${TESTDATA_1}" | vault write kv/buildkite/env/TESTDATA_1 value=-
   echo "${TESTDATA_2}" | vault write kv/buildkite/${PROJECT}/env/TESTDATA_2 value=-
}
